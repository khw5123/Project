...
def main():
    if len(sys.argv) == 1:
        create_key('key.pkr', 'key.skr', True)
        return
    elif len(sys.argv) != 3:
        print 'Usage : makeRSAKey.py PublickeyFile PrivatekeyFile'
        return
    create_key(sys.argv[1], sys.argv[2], True) # key.pkr(public key file), key.skr(private key file)
