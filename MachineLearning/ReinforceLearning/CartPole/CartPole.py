...
input_size = 4 # 입력 노드 개수(env.observation_space.shape[0])
output_size = 2 # 출력 노드 개수(env.action_space.n) (좌,우)
...
def replay_train(mainDQN, targetDQN, replay_buffer_minibatch): # 학습 함수
    ...
    for state, action, reward, next_state, done in replay_buffer_minibatch:
        ...
        x_stack = np.vstack([x_stack, state]) # 환경 스택에 쌓음
    ...
    with tf.Session() as sess: # 세션 생성
        ...
        for episode in range(epoch):
            ...
            while not done:
                ...
                if step_count > 10000: # 특정 반복 횟수를 초과했을 경우(막대기가 넘어지지 않고 오래 버텼을 경우)
                    break
            print('Episode: {}, steps: {}'.format(episode, step_count))
            if step_count > 10000: # 특정 반복 횟수를 초과했을 경우(막대기가 넘어지지 않고 오래 버텼을 경우)
                break
            if episode % 10 == 1: # 10번 반복했을 경우
                for _ in range(50):
