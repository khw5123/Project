...
# https://m.blog.naver.com/samsjang/220658244422
# http://darkpgmr.tistory.com/64

# Mean Shift : 어떤 데이터 분포의 무게중심을 찾는 방법으로 현재 자신의 주변에서 가장 데이터가 밀집된 방향으로 이동해서 언젠가는 분포의 중심을 찾는 알고리즘
# Histogram Backprojection : 현재 입력 영상에 있는 픽셀 색상 값이 추적하고자 하는 객체 모델에 얼마나 많이 포함되어 있는 색인지를 수치화하는 과정
...
def meanShift(device_index):
    ...
    termination = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 1) # meanShift 의 iteration 횟수가 10회일 때까지 알고리즘 구동 or 현재 범위의 중심과 무게중심의 차가 1pt 일 때까지 알고리즘 구동
    while True:
        ret, frame = cap.read() # 한 프레임 읽음
        ...
        if trackWindow is not None: # 추적 범위가 있을 경우
            ...
            ret, trackWindow = cv2.meanShift(dst, trackWindow, termination) # Mean Shift 알고리즘
            x, y, w, h = trackWindow # 추적 범위 구성요소 저장
            cv2.rectangle(frame, (x, y), (x+w, y+h), (0, 255, 0), 2) # 추적 범위 설정
